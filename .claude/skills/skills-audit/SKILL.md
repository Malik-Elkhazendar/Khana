...
name: skills-audit
description: >
  Audit the .claude/skills/ directory for dead skills, missing SKILL.md files,
  stale documentation references, and opportunities to consolidate. Run periodically
  to keep the skill system healthy.
# Claude Code extensions:
disable-model-invocation: true # must be triggered manually with /skills-audit
allowed-tools: Read, Glob, Grep, Bash
---

# Skills Audit

Perform a health check of the `.claude/skills/` directory and report findings.

## Steps

### 1. Inventory All Skills

Scan every subdirectory in `.claude/skills/`:
...
BROKEN (missing SKILL.md):
  - .claude/skills/<dir>/   ← no SKILL.md found

INVALID (bad frontmatter):
  - .claude/skills/<name>/SKILL.md  ← missing: description

STALE REFERENCES:
  - .claude/skills/<name>/SKILL.md:12  docs/authoritative/design/rtl.md
    → replace with: docs/DESIGN_SYSTEM.md or CLAUDE.md

OVERSIZED (> 500 lines):
  - .claude/skills/<name>/SKILL.md  (623 lines)
    → move detail to <name>/reference.md

BROKEN SUPPORTING FILES:
  - .claude/skills/<name>/SKILL.md references missing.md (not found)

POTENTIALLY DEAD:
  - .claude/skills/<name>/  references removed module: <module>

REDUNDANT WITH CLAUDE.md:
  - .claude/skills/<name>/SKILL.md §3 duplicates CLAUDE.md §SignalStore pattern

HEALTHY:
  - .claude/skills/project-guardrails/  ✓
