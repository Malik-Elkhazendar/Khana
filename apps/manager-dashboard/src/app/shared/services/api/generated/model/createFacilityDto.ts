/**
 * Generated by orval v6.14.1 🍺
 * Do not edit manually.
 * Khana API
 * Internal OpenAPI documentation for validating and testing Khana API endpoints before frontend integration.
 * OpenAPI spec version: 1.0
 */
import type { CreateFacilityConfigDto } from './createFacilityConfigDto';

export interface CreateFacilityDto {
  /** Display name for the facility. */
  name: string;
  /** Facility type label used by the tenant. */
  type: string;
  /** Pricing and operating-hours configuration for the facility. */
  config: CreateFacilityConfigDto;
}
